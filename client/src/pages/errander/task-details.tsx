...
export default function ErranderTaskDetails() {
  ...
  // Mock verification status (In a real app, this comes from user context)
  const isVerified = false; // Set to false to test the gatekeeper

  // Mock task data (in a real app, this would come from an ID or query param)
  const task = {
    title: "Repair Errands",
    category: "Repair Errands",
    posted: "May 30, 2025 at 06:30 PM",
    preferredStart: "May - Fri, at 09:28 PM",
    description: "Need help fixing a neon sign at my shop. The 'SHANGHAI' sign has a flickering 'G'. Need it done tonight if possible.",
    image: "https://images.unsplash.com/photo-1563245372-f21724e3856d?ixlib=rb-4.0.3&auto=format&fit=crop&w=600&h=800&q=80", // Neon sign placeholder
    budget: "KES 400 - 900",
    customer: {
        name: "Alex Kemboi",
  ...
  if (step === "submitted") {
    return <BidSubmittedView task={task} onSimulateAccept={simulateCustomerAcceptance} />;
  }

  if (step === "bid") {
    return <BidFormView task={task} onBack={() => setStep("view")} onSubmit={handleBidSubmit} />;
  }
...
function BidFormView({ task, onBack, onSubmit }: any) {
  return (
    <MobileLayout hideNav>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input type="number" placeholder="400" className="pl-12 h-12 rounded-xl text-lg font-bold" />
                </div>
              </div>
              <div className="space-y-2">
                <Label>Maximum Price</Label>
                 <div className="relative">
                  <span className="absolute left-3 top-3 text-gray-500 font-bold">KES</span>
                  <Input type="number" placeholder="900" className="pl-12 h-12 rounded-xl text-lg font-bold" />
                </div>
              </div>
            </div>
            <p className="text-xs text-gray-500">Suggested budget: {task.budget}</p>
          </div>

          <div className="space-y-2">
            <Label className="font-bold text-lg">Estimated Duration</Label>
            <div className="relative">
              <Clock className="absolute left-3 top-3 text-gray-500" size={20} />
              <Input type="text" placeholder="e.g. 2-3 hours" className="pl-10 h-12 rounded-xl" />
            </div>
            <p className="text-xs text-gray-500">How long will this take you to complete?</p>
          </div>
          
          <div className="space-y-2">
            <Label className="font-bold text-lg">Add a Note (Optional)</Label>
            <Input placeholder="I have the tools for this..." className="h-12 rounded-xl" />
          </div>
        </div>

        <div className="p-4 border-t">
          <Button onClick={onSubmit} className="w-full h-12 rounded-xl font-bold shadow-lg shadow-primary/20">
            Submit Proposal
          </Button>
        </div>
      </div>
...
function BidSubmittedView({ task, onSimulateAccept }: any) {
  const [, setLocation] = useLocation();
        ...
        <div className="p-6">
          <div className="flex justify-between items-start mb-6">
             <div className="flex gap-2">
               ...
               <span className="bg-yellow-100 text-yellow-700 text-xs font-bold px-3 py-1.5 rounded-full">Bid Pending</span>
             </div>
             <div className="text-right">
               <p className="text-xs text-gray-500 uppercase font-bold">Your Bid</p>
               <p className="text-green-600 font-bold text-lg">Kes: 400.00 - 900.00</p>
             </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-4">
             <div className="p-4 border-b border-gray-100">
               ...
               <p className="text-sm text-gray-500">Your bid is pending</p>
             </div>
             ...
             <div className="p-4 border-b border-gray-100">
               ...
               <div className="space-y-2 text-sm">
                 ...
                 <div className="flex justify-between">
                   ...
                   <span className="font-medium text-gray-900">May 30, 2025</span>
                 </div>
                 <div className="flex justify-between">
                   <span className="text-gray-500">Preferred start:</span>
                   <span className="font-medium text-gray-900">May - Fri, at 09:28 PM</span>
                 </div>
                  <div className="flex justify-between">
                   <span className="text-gray-500">Last Updated:</span>
                   <span className="font-medium text-gray-900">8 months ago</span>
                 </div>
               </div>
             </div>
             ...
             <div className="p-4">
                ...
                <div className="space-y-4 relative pl-4 border-l-2 border-gray-100 ml-1">
                   <div className="relative">
                     ...
                     <p className="text-xs text-gray-500 mb-0.5">Posted on May 30, 2025 at 06:30 PM</p>
                   </div>
                   <div className="relative">
                     ...
                     <p className="text-sm font-medium text-gray-900">You bid (KES): 400.00 - 900.00</p>
                     <p className="text-xs text-gray-500">on May 30, 2025 at 06:56 PM</p>
                   </div>
                </div>
             </div>
          </div>
